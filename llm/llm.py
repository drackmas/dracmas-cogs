            ...
            async with discord_channel.typing():
                async for token in response_stream:
                    if token.get("type") == "new_chunk":
                        # Flush existing contents safely
                        async with edit_lock:
                            if state.pending_content:
                        ...
                        # Create the new message container outside the lock to avoid deadlocks
    ...
    async def on_message_without_command(self, message: discord.Message):
        ...
        async with message.channel.typing():
            try:
                ...
                # Upgraded to print explicit class identifiers for tracking hidden bugs
